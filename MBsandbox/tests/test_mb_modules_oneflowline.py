...
def test_mb_modules_monthly():
    ...
    for k, clim in zip([0, 1], ['ERA5', 'ERA5dr']):
        ...
        mb_mod = mb_modules(gdir, mu_opt, mb_type='mb_monthly',
            ...
            for grad_type in ['cte', 'var_an_cycle']:
                ...
                if fail_err_1 or fail_err_2 or fail_err_3 or fail_err_4:
                    ...
                    for i, yr in enumerate(np.arange(yrp[0], yrp[1]+1)):
                        ...
                        assert_allclose(np.mean(my_an_mb_on_h-my_mon_mb_on_h),
                                        0, atol=100)
...
def test_loop():
    ...
    for grad_type in ['cte', 'var_an_cycle']:
        ...
        if fail_err_4:
            with pytest.raises(InvalidParamsError):
                mb_modules(gdir, mu_star_opt[mb_type],
                           mb_type=mb_type, prcp_fac=pf,
                           t_solid=0, t_liq=2, t_melt=0,
                           default_grad=-0.0065, bias=0,
                           grad_type=grad_type)
            ...
            assert_allclose(tot_mb_loop, tot_mb_noloop,
                            atol=1e-2)
