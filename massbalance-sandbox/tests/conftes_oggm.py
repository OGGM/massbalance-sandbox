"""FROM OGGM default, but not used at the moment ...Pytest fixtures to be used in other test modules"""
...
def secure_url_retrieve(url, *args, **kwargs):
    ...
    assert ('github' in url or
            ...
            'cluster.klima.uni-bremen.de/~fmaussion' in url or  # LILY added
            'cluster.klima.uni-bremen.de/~lschuster' in url or # LILY added
            'cluster.klima.uni-bremen.de/~oggm/climate/era5/'
            )
...
def _setup_case_dir(call, test_dir):
    ...
    # name = call.__name__
    name = 'blub'
    casedir = os.path.join(test_dir, name)
...
def class_case_dir(request, test_dir):
    ...
    """
    cd = _setup_case_dir(request.cls, test_dir) # cls centerlines 
    yield cd
    _teardown_case_dir(cd)
